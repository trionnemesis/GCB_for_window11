...
* **目前最新版本**：TWGCB-01-010 **v1.0**（中華民國112年12月／1121201），截至 2026-07 官方未發布更新版本。

---
## 變更紀錄（複查）

- **2026-07-03 基準版本複查**：依 NICS 官方文件下載索引與多筆獨立網路搜尋結果交叉確認，Microsoft Windows 11 之政府組態基準**目前最新公告版本仍為 `TWGCB-01-010` v1.0**（中華民國112年12月／1121201）。**未找到任何 v1.1 版本文件或公告**（包含檔名索引與搜尋引擎皆無 `TWGCB-01-010 v1.1` 或 `GCB-Windows11-gpos v1.2 (1141105)` 的紀錄）。
  - 本次審查受限於此執行環境之出口網路政策（`nics.nat.gov.tw`、`download.nics.nat.gov.tw` 皆回應 HTTP 403 / host not allowed），未能直接下載官方 PDF 逐項比對內容；版本存在性係以官方文件下載連結檔名索引與多筆公開搜尋結果交叉驗證。
  - ⚠️ **開放 PR 版本主張衝突提醒**：本 repo 目前有兩個互相衝突的開放草稿 PR：
    - **PR #2**「對齊 Windows 11 GCB 工具至 TWGCB-01-010 v1.1 (1141105)」主張存在 v1.1 版本，但其自身描述亦承認因網路政策無法連線 NICS 下載 PDF 逐項驗證。本次獨立複查同樣無法連線 NICS，但透過官方文件檔名索引與多筆搜尋結果交叉比對，**均未發現 v1.1 版本存在的證據**，故 PR #2 的版本升級主張**目前無法證實**，建議在取得官方 v1.1 PDF 佐證前暫緩合併。
    - **PR #3**「修正 Windows 11 GCB 帳戶原則範圍比對邏輯」正確地將基準版本標示維持在 v1.0，並修正 `Check-Set-SecurityPolicy` 原本以完全相等 (`-eq`) 誤判「更嚴格但已合規」設定、且會將設定弱化至門檻值的邏輯錯誤（改用 `AtLeast`/`AtMostNonZero` 語意）。此修正與本次獨立驗證結果一致，建議優先審閱合併。
  - 本次變更僅為文件層級之版本複查記錄，未修改 `GCB_for_windows11.ps1` 的任何邏輯或規則期望值。
