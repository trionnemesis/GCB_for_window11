...
本 PowerShell 指令碼旨在協助系統管理員根據國家資通安全研究院發布的「**Microsoft Windows 11 政府組態基準說明文件 (TWGCB-01-010) v1.1 (1141105)**」，自動化地檢測與修正本機設定。
...
* **政府組態基準 (GCB) 文件**: `TWGCB-01-010_Microsoft Windows 11政府組態基準說明文件v1.1_1141105.pdf`
* **部署基準 (GPO)**: `GCB-Windows11-gpos v1.2 (1141105)`
* **來源**: 國家資通安全研究院 (NICS) — [GCB 說明文件](https://www.nics.nat.gov.tw/core_business/cybersecurity_defense/GCB/GCB_Documentation/)、[更新消息](https://www.nics.nat.gov.tw/core_business/cybersecurity_defense/GCB/Latest_News/)

---
## 變更紀錄

### v1.1 (2026-07-01) — 對齊 TWGCB-01-010 v1.1

依 NICS 於中華民國114年11月5日 (1141105) 發布之最新版說明文件，將本工具之基準版本標示自 **v1.0 (1121201)** 更新為 **v1.1 (1141105)**：

* 更新 `README.md` 與 `GCB_for_windows11.ps1` 中之文件版本、日誌標頭與參考文件連結。
* 標註對應之部署基準 `GCB-Windows11-gpos v1.2 (1141105)`。
* v1.1 之主要調整方向為「**帳戶原則與更細緻的密碼原則**」(含帳戶鎖定閾值、鎖定期間與密碼原則)，對應 Windows 11 較新版本 (含 24H2) 的預設帳戶鎖定行為。

> ⚠️ **待人工核對 (Action required)**
> 本次更新受限於自動化環境之對外連線政策，**無法自 NICS 直接下載 v1.1 PDF 逐項比對**，因此僅更新版本標示與文件說明，尚未變更任何個別項目之期望值。請維運人員取得官方 v1.1 說明文件後，重點覆核下列項目並視差異調整腳本：
> - **帳戶原則 / 密碼原則**：`TWGCB-01-010-0001`~`0007` (最短/最長使用期限、最小長度、複雜性、帳戶鎖定閾值/鎖定期間/重設鎖定計數器)。
> - 是否新增 v1.0 未涵蓋之項目 (v1.0 共 397 項；請確認 v1.1 之項數統計與新增/移除清單)。
> 本腳本本質為**範本框架**，僅實作部分代表性項目，並非完整 397 項；擴充方式請見上方「指令碼擴充說明」。
