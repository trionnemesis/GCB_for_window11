...
* **來源**：國家資通安全研究院 (NICS) — https://www.nics.nat.gov.tw/ ／ GCB 說明文件下載頁面
* **目前最新版本**：TWGCB-01-010 **v1.0**（中華民國112年12月/2023-12），共 156 項基準項目。截至 2026-07 官方尚未發布更新版本。

## 變更紀錄

* **2026-07-02**：對齊並確認現行 NICS 基準版本 TWGCB-01-010 v1.0。
  * 修正 `Check-Set-SecurityPolicy` 對帳戶原則的比對邏輯：GCB 的帳戶原則為「範圍」而非「固定值」，原本以完全相等 (`-eq`) 判斷會將**已符合但更嚴格**的設定誤判為不合規，並在修正時把設定**弱化**至門檻值。新增 `-Comparison` 參數（`Equal` / `AtLeast` / `AtMostNonZero`）：
    * `TWGCB-01-010-0001 密碼最短使用期限`：≥ 1 天 (`AtLeast`)
    * `TWGCB-01-010-0002 密碼最長使用期限`：1~90 天、不可為 0 (`AtMostNonZero`)
    * `TWGCB-01-010-0003 最小密碼長度`：≥ 8 字元，保留更長的既有設定 (`AtLeast`)
    * `TWGCB-01-010-0007 帳戶鎖定閾值`：1~5 次、不可為 0 (`AtMostNonZero`)
  * 修正主程式註解中錯誤的項目總數（原標示 397，實際為 156）。
